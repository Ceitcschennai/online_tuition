...
const Assignments = () => {
  ...
  // ✅ Unique subjects present in the fetched assignments, for the filter tabs
  const subjectTabs = ["All", ...Array.from(
    new Set(assignments.map((a) => a.subject).filter(Boolean))
  )];

  // ✅ Only show assignments for the selected subject
        ...
        <div style={{
          ...
          <p style={{ fontSize: 18 }}>
            {selectedSubject === "All"
              ...
              : `No ${selectedSubject} assignments available.`}
          </p>
          {studentInfo && (
            <p style={{ fontSize: 13, marginTop: 8 }}>
              Class: <strong>
