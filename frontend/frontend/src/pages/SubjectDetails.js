...
// Build a case-insensitive lookup so "maths", "MATHS", or " Maths " all
// resolve to the same entry as the canonical key "Maths".
const subjectLookup = Object.keys(subjectData).reduce((acc, key) => {
  acc[key.toLowerCase()] = key;
  return acc;
}, {});

// If there's no exact (case-insensitive) match, suggest subjects whose name
// contains the search text or vice-versa, e.g. "chem" -> "Chemistry".
const getSuggestions = (rawText) => {
  const query = rawText.trim().toLowerCase();
  if (!query) return [];
  return Object.keys(subjectData).filter(
...
const SubjectDetails = () => {
  ...
  const canonicalKey = subjectLookup[normalized];
  const data = canonicalKey ? subjectData[canonicalKey] : undefined;

  if (!data) {
    const suggestions = getSuggestions(subjectName || '');

    return (
      <div className="sd-not-found">
        <h2>Subject not found</h2>
        <p>We couldn't find a course matching "{subjectName}".</p>
        ...
        <div className="sd-not-found-actions">
          <button onClick={() => navigate(-1)}>Go Back</button>
          <button className="sd-browse-all-btn" onClick={() => navigate('/subjects')}>
            Browse all subjects
          </button>
        </div>
      </div>
