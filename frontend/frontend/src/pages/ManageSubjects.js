...
const ManageSubjects = () => {
  ...
  const [teachers, setTeachers] = useState([]); // ✅ approved teachers for the dropdown
  ...
  const fetchSubjects = useCallback(async () => {
    ...
    try {
      ...
      // ✅ Was calling GET /api/subjects, which requires a `class` param and
      // returns an empty list without one — this page never actually showed
      // any subjects. Use the dedicated admin listing endpoint instead.
      const response = await fetch(`${API_BASE_URL}/api/subjects/admin/all?${queryParams}`);
  ...
  // ✅ Fetch approved teachers for the assignment dropdown
  const fetchTeachers = useCallback(async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/admin/teachers?filter=approved`);
      const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSend = {
        ...
        teacher: formData.teacher || null, // ✅ assigned teacher
      };
