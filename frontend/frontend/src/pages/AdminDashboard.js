      ...
      <div className="aqc-right">
          ...
          {role === 'student' && (
            <div className="aqc-file-row">
              <label className="aqc-file-label">
                🪪 {certFile ? 'Change ID Proof' : 'Upload Student ID'}
        ...
        <details className="aqc-guide">
          ...
          <ol className="aqc-guide-list">
            ...
            <li>
              <strong>{role === 'student' ? 'Student ID' : 'Degree Certificate'}</strong>
...
// ✅ Now requires BOTH name and class — far more reliable than name-only lookup,
// and a student with zero attendance records is shown as a valid 0%, not "not found".
const StudentAttendanceSearch = () => {
  ...
  const fetchSuggestions = async (val) => {
    ...
    try {
      const res  = await fetch(`${API_BASE_URL}/api/student/search?name=${encodeURIComponent(val)}`);
      const data = await res.json();
...
const AdminDashboard = () => {
  ...
  const fetchPendingStudents = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/student/admin/pending`);
  ...
  const fetchPendingTeachers = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/teacher/admin/pending`);
  ...
  const handleStudentApproval = async (status) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE_URL}/api/student/admin/${selectedStudent._id}/approve`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="admin-dashboard">
      ...
      {pendingStudents.length === 0 ? (
        ...
        pendingStudents.map((student) => (
          <React.Fragment key={student._id}>
            <div
              className={`card ${selectedStudent?._id === student._id ? 'card-active' : ''}`}
              onClick={() => {
      ...
      {pendingTeachers.length === 0 ? (
        ...
        pendingTeachers.map((teacher) => (
          <React.Fragment key={teacher._id}>
            <div
              className={`card ${selectedTeacher?._id === teacher._id ? 'card-active' : ''}`}
              onClick={() => {
      ...
      <p className="admin-section-subtitle">Search any student by name and class to view their attendance percentage</p>
      <StudentAttendanceSearch />
